...
export async function archiveRelation(aid: number): Promise<ArchiveRelation> {
  return invoke("archive_relation", { aid });
}
...
export async function playerOpen(
  ...
  return invoke("player_open", {
    req: { bvid, cid, quality, scope: "player" },
  });
}
...
export async function playerStop(): Promise<void> {
  return invoke("player_stop", { scope: "player" });
}

export async function playerStopBackdrop(): Promise<void> {
  return invoke("player_stop", { scope: "featured" });
}
...
export async function playerSetDanmakuPrefs(
  fontSize?: number,
  maxRows?: number,
): Promise<void> {
